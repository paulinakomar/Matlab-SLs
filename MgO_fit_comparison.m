...
figure('Units', 'centimeters', 'Position', [5 2 30 22]);
    subplot(2,2,1)
        [gauss_fit, gof_gauss, output_gauss]       = MgO_gauss(x,y);
        res_gauss = 0;
    ...
    subplot(2,2,2)
        [lorentz_fit, gof_lorentz, output_lorentz] = MgO_Lorentz(x,y);
        res_lorentz = 0;
    ...
    subplot(2,2,3)
        [voight_fit, gof_voight, output_voight]    = MgO_Voight(x,y);
        res_voight = 0;
    ...
    subplot(2,2,4)
        [pearson_fit, gof_pearson, output_pearson] = MgO_PearsonVII(x,y);
        res_pearson = 0;
