...
%% data for SL sumulation
buffer_YN = 1;
buffer_material = 1;
Nlayers_buffer = 1800;
range(1,1) = 29.3; range(1,2) = 0.005; range(1,3) = 30.6;
range(2,1) = 61.2; range(2,2) = 0.005; range(2,3) = 63.5;
system = 1;
N_bilayer = 0;
N_EZ_a = 21.75;
N_EZ_b = 21.5;
...
%% Pseudo Voight parameters for MgO
...
for i = 1:length(twotheta_200)
    ...
    voight_fit_200 = y0_200 + (AG_200/(wG_200*sqrt(2*pi)))*exp(-((twotheta_200-center_200)/(sqrt(2)*wG_200)).^2) +...
...
for i = 1:length(twotheta_400)
    ...
    voight_fit_400 = y0_400 + (AG_400/(wG_400*sqrt(2*pi)))*exp(-((twotheta_400-center_400)/(sqrt(2)*wG_400)).^2) +...
...
% for i = 1:length(twotheta_200)
%     center_200 = twotheta_200(i);
%     voight_fit_200 = y0 + (AG/(wG*sqrt(2*pi)))*exp(-((twotheta_200-center_200)/(sqrt(2)*wG)).^2) +...
%         (AL/pi)*(wL./((twotheta_200-center_200).^2 + wL^2)) +...
%         (AG2/(wG2*sqrt(2*pi)))*exp(-((twotheta_200-center_200)/(sqrt(2)*wG2)).^2) +...
%         (AL2/pi)*(wL2./((twotheta_200-center_200).^2 + wL2^2));
%     IntensityVoight_200(i) = voight_fit_200*(Intensity_200_lambda1' + 0.5*Intensity_200_lambda2');
% end
% 
% for i = 1:length(twotheta_400)
%     center_400 = twotheta_400(i);
%     voight_fit_400 = y0 + (AG/(wG*sqrt(2*pi)))*exp(-((twotheta_400-center_400)/(sqrt(2)*wG)).^2) +...
%         (AL/pi)*(wL./((twotheta_400-center_400).^2 + wL^2)) +...
%         (AG2/(wG2*sqrt(2*pi)))*exp(-((twotheta_400-center_400)/(sqrt(2)*wG2)).^2) +...
%         (AL2/pi)*(wL2./((twotheta_400-center_400).^2 + wL2^2));
%     IntensityVoight_400(i) = voight_fit_400*(Intensity_400_lambda1' + 0.5*Intensity_400_lambda2');
% end
...
figure('Units', 'centimeters', 'Position', [5 2 39 39]);
subplot(2,1,1) % Ka1 + Ka2, pseudo Voight
    h1 = semilogy(twotheta_200, IntensityVoight_200, 'r', 'Linewidth', 1.5); hold on;
    h2 = semilogy(twotheta_400, IntensityVoight_400, 'r', 'Linewidth', 1.5);
    ...
    h4 = semilogy(SL_meas_400(:,1), SL_meas_400(:,2)/max(SL_meas_400(:,2)), ':.k');
        ...
        set(gca, 'fontsize', 13);
        grid on; box on;
    legend([h1 h3],{['Pseudo Voight fit' char(10) 'K_{\alpha}_1 + K_{\alpha}_2'], 'Measured data'}, 'location', 'south');%, 'Measured data');
    breakxaxis([33 56], 0.005);
    
            
subplot(2,1,2) % Ka1, Gauss
    h1 = semilogy(twotheta_200,IntensityGauss1, 'r', 'Linewidth', 1.5); hold on;
    h2 = semilogy(twotheta_400,IntensityGauss2, 'r', 'Linewidth', 1.5);
    h3 = semilogy(SL_meas_200(:,1), SL_meas_200(:,2)/max(SL_meas_200(:,2)), ':.k');
    h4 = semilogy(SL_meas_400(:,1), SL_meas_400(:,2)/max(SL_meas_400(:,2)), ':.k');
        xlabel( '{\it{2\theta}} (°)' , 'fontsize', 15);
        set(gca, 'XTick', 25:2:66);
        ...
        set(gca, 'fontsize', 13);
        grid on; box on;
    legend([h1 h3],{'Gaussian broadening', 'Measured data'}, 'location', 'south');%, 'Measured data');
    breakxaxis([33 56], 0.005);
